...
Nineteen problems are registered — one per day of work, plus the Week 1 3-DoF
optimiser. The Day 18 entry is the only one solving a problem this project did
not choose: it reproduces a published benchmark, and gives you the two controls
that break the reproduction — swap the quadrature to a single-endpoint
approximation, or push the mass constant to the value the source guide calls
harmless, and watch a machine-precision answer become a runaway.
The Day 17 entry is where the 3-D work gets checked: flip its
...
| Day 18 | Benchmark replication: Szmuk & Açıkmeşe (2018), reproduced | done |
...
**The physics matches, independently derived.** The paper's rigid-body model
and this project's turn out to be the same physics written twice: direction
cosine matrices agreeing to **8.88e-16** over 400 random quaternions, and the
gyroscopic term matching Day 14's to **0.00e+00**. That is the strongest
confirmation Days 13–15 have had.

**And the paper's central claim reproduces.** Its headline is robustness: ten
time-of-flight guesses from 1 to 10 UT all converging within **0.01 UT**.
Measured across all ten here, the spread is **0.00183 UT** at
tf = 3.28159 ± 0.00052, with virtual control between 1e-15 and 1e-17 — and that
is with Algorithm 1 **exactly as printed**, no hard trust region and no
quaternion renormalisation, both of which the Day 18 guide calls necessary
additions. Neither is. The hard box measurably *degrades* the sweep, 0.025 UT
of spread against 0.002 without it.

**Two things had to be right, and they are separable.** The guide sets `α_ṁ` —
a mass constant the paper never gives — to 1.0 and calls it a harmless
placeholder, then blames its whole residual on using a cruder discretisation
than the paper's. Both halves are wrong, and the 2×2 settles it:

| | `α_ṁ = 1.0` | `α_ṁ = 0.03` |
|---|---|---|
| single-endpoint quadrature | 4.94 | 4.19 |
| the paper's Eq. 22 integrals | 5.41 | **2.06e-16** |

Only the corner with both corrected converges. At `α_ṁ = 1.0` a three-unit
trajectory needs **6 UM of propellant against the 1 UM the vehicle carries** —
the mass floor binds and no quadrature repairs a constraint that is simply
active. With `α_ṁ` sized properly, the residual is 100% in the position row,
which is what a discretisation limit actually looks like, and the paper's own
integrals then close it to machine precision. The state transition matrix is
identical between the two schemes; they differ by **4.6% on one input matrix**.

**A correction, recorded rather than quietly swapped in.** An earlier version
of this section reported that the paper's claim did *not* reproduce, and
offered the paper's cost weights as the reason. That was wrong. The
single-endpoint quadrature was this implementation's shortcut, not the paper's,
and it was what broke the sweep — I had a real measurement and attached it to
the wrong cause, having named the missing piece in the same write-up and
shipped the conclusion anyway. Full correction in [LOG.md](LOG.md).
